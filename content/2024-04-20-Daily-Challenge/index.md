+++
updated = 2024-04-20T12:30:20+08:00
title = "2024-04-20 Daily Challenge"
path = "2024-04-20-Daily-Challenge"
date = 2024-04-20T12:30:20+08:00
...
Today I have done leetcode's [April LeetCoding Challenge](https://leetcode.com/problems/find-all-groups-of-farmland/) with `cpp`.
...
``` cpp
class Solution {
  vector<int> getFarmLand(const vector<vector<int>>& land, int top, int left) {
    int m = land.size();
    int n = land.front().size();
    int bot = top;
    int right = left;
    for(int i = top; i < m && land[i][left]; ++i) {
      bot = i;
    }
    for(int i = left; i < n && land[top][i]; ++i) {
      right = i;
    }
  ...
  vector<vector<int>> findFarmland(vector<vector<int>>& land) {
    ...
    for(int i = 0; i < m; ++i) {
      for(int j = 0; j < n; ++j) {
        ...
        if((i && land[i - 1][j]) || (j && land[i][j - 1])) continue;
        answer.push_back(getFarmLand(land, i, j));
      }
    }
    return answer;
  }
