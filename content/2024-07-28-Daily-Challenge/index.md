+++
updated = 2024-07-28T14:37:13+08:00
title = "2024-07-28 Daily Challenge"
path = "2024-07-28-Daily-Challenge"
date = 2024-07-28T14:37:13+08:00
...
Today I have done leetcode's [July LeetCoding Challenge](https://leetcode.com/problems/second-minimum-time-to-reach-destination/) with `cpp`.
...
``` cpp
class Solution {
  int totalTime(int step, int time, int change) {
    int answer = 0;
    for(int i = 0; i < step; ++i){
      int gr = answer / change;
      if(gr & 1) {
        answer = (gr + 1) * change;
      }
      answer += time;
  ...
  int secondMinimum(int n, vector<vector<int>>& edges, int time, int change) {
    ...
    while(q.size()) {
      ...
      for(auto next : graph[cur]) {
        int newDis = dis + 1;
        if(newDis < dist1[next]) {
          dist2[next] = dist1[next];
          dist1[next] = newDis;
          q.push({next, newDis});
        } else if(newDis > dist1[next] && newDis < dist2[next]) {
          dist2[next] = newDis;
          q.push({next, newDis});
        }
      }
    ...
    int steps = dist2.back();
    if(steps == INT_MAX) return -1;
    return totalTime(steps, time, change);
  }
};
```
