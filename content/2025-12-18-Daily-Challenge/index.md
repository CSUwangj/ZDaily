+++
updated = 2025-12-18T21:23:28+01:00
title = "2025-12-18 Daily Challenge"
path = "2025-12-18-Daily-Challenge"
date = 2025-12-18T21:23:28+01:00
...
Today I have done leetcode's [December LeetCoding Challenge](https://leetcode.com/problems/best-time-to-buy-and-sell-stock-using-strategy/) with `cpp`.
...
public:
  long long maxProfit(vector<int>& prices, vector<int>& strategy, int k) {
    ...
    vector<long long> prefix(days + 1);
    vector<long long> original(days + 1);
    for(int i = 0; i < days; ++i) {
      prefix[i + 1] = prefix[i] + prices[i];
      original[i + 1] = original[i] + prices[i] * strategy[i];
    }

    long long nomodify = original.back();
    long long answer = nomodify;
    for(int i = 0; i + k <= days; ++i) {
      answer = max(answer, nomodify + prefix[i + k] - prefix[i + k / 2] - original[i + k] + original[i]);
    }
    return answer;
  }
};
...
// Accepted
// 976/976 cases passed (28 ms)
// Your runtime beats 17.19 % of cpp submissions
// Your memory usage beats 33.59 % of cpp submissions (328.1 MB)
```
