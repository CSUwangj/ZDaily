+++
updated = 2023-10-18T20:04:30+08:00
title = "2023-10-18 Daily Challenge"
path = "2023-10-18-Daily-Challenge"
date = 2023-10-18T20:04:30+08:00
...
Today I have done leetcode's [October LeetCoding Challenge](https://leetcode.com/problems/parallel-courses-iii/) with `cpp`.
...
public:
  int minimumTime(int n, vector<vector<int>>& relations, vector<int>& time) {
    ...
    while(pq.size()) {
      auto [finishTime, course] = pq.top();
      pq.pop();
      answer = max(finishTime, answer);
      for(auto next : next[course]) {
        degree[next] -= 1;
        if(degree[next]) continue;
        visit[next] = true;
        pq.push({finishTime + time[next], next});
      }
    }
...
// Accepted
// 42/42 cases passed (297 ms)
// Your runtime beats 92.81 % of cpp submissions
// Your memory usage beats 88.31 % of cpp submissions (131.2 MB)
```
