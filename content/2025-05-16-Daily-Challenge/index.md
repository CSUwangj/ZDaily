+++
updated = 2025-05-17T02:32:16+08:00
title = "2025-05-16 Daily Challenge"
path = "2025-05-16-Daily-Challenge"
date = 2025-05-17T02:32:16+08:00
...
Today I have done leetcode's [May LeetCoding Challenge](https://leetcode.com/problems/longest-unequal-adjacent-groups-subsequence-ii/) with `cpp`.
...
public:
  vector<string> getWordsInLongestSubsequence(vector<string>& words, vector<int>& groups) {
    unordered_map<string, unordered_set<string>> valid;
    for(auto w : words) {
      for(auto ww : words) {
        if(w.length() != ww.length()) continue;
        int count = 0;
    ...
    int len = words.size();
    vector<int> from(len, -1);
    vector<int> length(len, 1);
    for(int i = 1; i < len; ++i) {
      for(int j = 0; j < i; ++j) {
        if(groups[i] == groups[j]) continue;
        if(valid[words[i]].count(words[j]) && length[i] < length[j] + 1) {
          from[i] = j;
          length[i] = length[j] + 1;
        }
      }
    ...
    int current = distance(length.begin(), max_element(length.begin(), length.end()));
    while(current != -1) {
      answer.push_back(words[current]);
...
// Accepted
// 441/441 cases passed (429 ms)
// Your runtime beats 5.69 % of cpp submissions
// Your memory usage beats 22.76 % of cpp submissions (58.5 MB)
```
