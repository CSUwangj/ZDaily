+++
updated = 2024-11-06T14:17:23+08:00
title = "2024-11-06 Daily Challenge"
path = "2024-11-06-Daily-Challenge"
date = 2024-11-06T14:17:23+08:00
...
Today I have done leetcode's [November LeetCoding Challenge](https://leetcode.com/problems/find-if-array-can-be-sorted/) with `cpp`.
...
public:
  bool canSortArray(vector<int>& nums) {
    ...
    for(int i = 0; i < nums.size(); ) {
      int range;
      for(range = 1; i + range < nums.size() && __builtin_popcount(nums[i + range]) == __builtin_popcount(nums[i + range - 1]); ++range);
      sort(nums.begin() + i, nums.begin() + i + range);
      i += range;
    }
    for(int i = 0; i < nums.size(); ++i) if(nums[i] != sorted[i]) return false;
    return true;
  }
};
```
