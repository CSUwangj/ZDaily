+++
updated = 2024-12-17T15:35:18+08:00
title = "2024-12-17 Daily Challenge"
path = "2024-12-17-Daily-Challenge"
date = 2024-12-17T15:35:18+08:00
...
Today I have done leetcode's [December LeetCoding Challenge](https://leetcode.com/problems/construct-string-with-repeat-limit/) with `cpp`.
...
public:
  string repeatLimitedString(string s, int repeatLimit) {
    ...
    while(pq.size()) {
      ...
      answer.append(use, current + 'a');

      freq[current] -= use;
      if(freq[current] > 0 && pq.size()) {
        char next = pq.top();
        pq.pop();
        answer.push_back(next + 'a');
        freq[next] -= 1;

        if(freq[next]) pq.push(next);
        pq.push(current);
      }
    }
    return answer;
