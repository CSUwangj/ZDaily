+++
updated = 2025-09-21T21:01:54+02:00
title = "2025-09-21 Daily Challenge"
path = "2025-09-21-Daily-Challenge"
date = 2025-09-21T21:01:54+02:00
...
Today I have done leetcode's [September LeetCoding Challenge](https://leetcode.com/problems/design-movie-rental-system/) with `cpp`.
...
``` cpp
constexpr long long shopmovie(long long shop, int movie) {
  return (shop << 32) | movie;
}
class MovieRentingSystem {
  using movie = tuple<int, int, int>;
  using store = pair<int, int>;
  map<long long, int> shopmovie2price;
  map<int, set<store>> movie2priceshop;
  set<movie> rented;
public:
  MovieRentingSystem(int n, vector<vector<int>>& entries) {
    for(auto &e : entries) {
      ...
      int movie = e[1];
      int price = e[2];
      shopmovie2price[shopmovie(shop, movie)] = price;
      movie2priceshop[movie].insert({price, shop});
    }
  }
  
  vector<int> search(int movie) {
    const auto &stores = movie2priceshop[movie];
    vector<int> answer;
    for(auto it = stores.begin(); it != stores.end() && answer.size() < 5; ++it) {
  ...
  void rent(int shop, int movie) {
    int price = shopmovie2price[shopmovie(shop, movie)];
    movie2priceshop[movie].erase({price, shop});
  ...
  vector<vector<int>> report() {
    ...
    for(auto it = rented.begin(); it != rented.end() && answer.size() < 5; ++it) {
...
// Accepted
// 42/42 cases passed (293 ms)
// Your runtime beats 74.45 % of cpp submissions
// Your memory usage beats 85.4 % of cpp submissions (401.1 MB)
```
